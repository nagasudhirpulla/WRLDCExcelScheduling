...
Sub fetch_generator_data()
    ...
    ClearConsole
    WriteLineConsole "From generator info fetch method..."
    ...
    WriteLineConsole "Fetched the generator data"
    ' Tidy up
    .Close
    End With
    
    cn.Close    'close connection to db
...
Sub update_generator_data()
    ...
    ClearConsole
    WriteLineConsole "From generator info update method..."
    ...
    Do While Trim(Sheets("GENERATORS").Cells(rowIterator, 1).Value) <> "" And rowIterator < 100
        ...
        ElseIf num > 1 Then
            WriteLineConsole "Beware: not updating a unique row at row " & CStr(rowIterator) & " of GENERATORS sheet"
        End If
        rowIterator = rowIterator + 1
    Loop
    
    If numRowsAffected > 0 Then
        WriteLineConsole "Updated " & numRowsAffected & " rows, " & msg & "of the GENERATORS sheet"
    Else
...
Sub create_or_update_generator_data()
    ...
    ClearConsole
    WriteLineConsole "From generator info create or update method..."
    ...
    With rsMaterialsdb
        ...
        Do While Trim(Sheets("GENERATORS").Cells(1, FieldColTick).Value) <> "" And canProceed = True
            ...
            Else
                canProceed = False
                WriteLineConsole "The field " & Trim(Sheets("GENERATORS").Cells(1, FieldColTick).Value) & " doesnot exist in database"
            End If
            FieldColTick = FieldColTick + 1 'tick iterator
        Loop
        ''Now FieldColTick is equal to number of fields
    ...
    Do While Trim(Sheets("GENERATORS").Cells(rowIterator, 1).Value) <> "" And rowIterator < 100
        ...
        ElseIf num > 1 Then
            WriteLineConsole "Beware: not updating a unique column at row " & CStr(rowIterator) & " of GENERATORS sheet"
        End If
        rowIterator = rowIterator + 1
    Loop

    If numRowsAffected > 0 Then
        WriteLineConsole "Updated " & numRowsAffected & " rows, " & updateMsg & "of the GENERATORS sheet"
    Else
        WriteLineConsole "Updated zero rows"
    End If
